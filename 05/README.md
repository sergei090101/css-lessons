...
Это внешняя таблица стилей, которую использует файл cascade.html. В ней уже
есть несколько стилей: первая группа сбрасывает стандартные стили.
Они устраняют основные стили браузера,
поэтому весь текст пока выглядит одинаково. Скоро вы будете создавать собственные
стили, чтобы эта страница смотрелась более эффектно.
Последние два стиля — классы .main и .sidebar — создают две колонки.
...
верхний заголовок.
3. В файле styles.css добавьте два этих стиля в нижней части таблицы стилей
